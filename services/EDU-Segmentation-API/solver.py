...
class TrainSolver(object):
    ...
    def check_accuracy(self, dataX, dataY):
        ...
        for lp in range(need_loop):
            ...
            ba_C, ba_R, ba_G = self.get_batch_micro_metric(
                batch_boundary, batch_y)
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            self.adjust_learning_rate(
                optimizer, epoch, 0.8, self.lr_decay_epoch)
            ...
            for iter in range(num_each_epoch):
                ...
                neg_loss = self.model.neg_log_likelihood(
                    batch_x, index_decoder_X, index_decoder_Y, all_lens)
            ...
            save_file_name = 'bs_{}_es_{}_lr_{}_lrdc_{}_wd_{}_epoch_loss_acc_pk_wd.txt'.format(
                self.batch_size, self.eval_size, self.lr, self.lr_decay_epoch, self.weight_decay)
            with open(os.path.join(self.save_path, save_file_name), 'a') as f:
                f.write(','.join(map(str, save_data))+'\n')

            if epoch % 1 == 0 and epoch != 0:
                torch.save(self.model, os.path.join(self.save_path,
                           r'model_epoch_%d.torchsave' % (epoch)))
